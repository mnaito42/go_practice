...
//引数の型指定をジェネリックに
func RemoveByIndex[T any](sl []T, indx int)([]T, string){ //どんな型でもよいT

	s_string := fmt.Sprintf("インデックスが%dの要素を除きます",indx)

	rest := []T{} //ただしTと書いてあるところは同じ型
	...
	return rest, s_string //s_stringは説明文
}

//並行処理の実験のため、遅らせて処理する
//引数timesは、繰り返し上何回目の出力かを示す
func Record(s string, times int, interval int) string {

	//time.Millisecondに直接整数は掛けられない
...
//チャンネルに値を送る
func InChannel(s string, times int, interval int, 
	ch chan string){ //string型の値を通すチャンネル

	ch<-Record(s, times, interval)//chに向いている矢印
}

//ファイルの読み書き
func ReadMyFile(filepath string) string{

	data, err := os.ReadFile(filepath) //エラーがあれば報告も受けとる
	if err != nil{ //「空」はnil
		return "ファイルを開けませんでした"
	}
	return string(data) //正常ならこれが戻る
}
...
func WriteMyFile(filepath string, content string) string{ //とにかく文字列を戻す
	err := os.WriteFile(filepath, []byte(content), 0666)//読み書き自由のアクセス制御
	if err != nil{
		return "ファイルを書き込めませんでした"
	}
	//このメッセージ自体はそれほど重要ではなく、ファイルができたか確認しましょう
