...
func files(writer http.ResponseWriter, req *http.Request){
	...
	readdata:= functions.ReadMyFile("data/readtest.txt") //読み込んだ内容を渡しておく
	fmt.Fprintln(writer,readdata)

	fmt.Fprintln(writer, "\n***ファイルの内容を１行ずつ分ける***")
	lines := functions.SplitByLine(readdata) //改行コードで分ける

	for i, v := range lines{ //インデックスと値を取得
		if len(v)>0{ //空の改行でなければ
	...
	fmt.Fprintln(writer,
		functions.WriteMyFile("data/writetest.txt", 
			writedata)) //このメッセージ自体はそれほど重要ではなく、ファイルを確認しましょう
